...
from app.data.team_matching import canonical_alias, name_similarity
...
class TeamIndex:
    ...
    def resolve(self, provider_name: str) -> Team | None:
        ...
        best: tuple[float, Team | None] = (0.0, None)
        for team in self._teams:
            score = name_similarity(provider_name, team.name)
            if score > best[0]:
                best = (score, team)
        return best[1] if best[0] >= _NAME_THRESHOLD else None
    ...
    *,
    ...
    for row in rows:
        ...
        if home is None or away is None:
            unresolved = home_name if home is None else away_name
            report.skipped_unresolved.append(f"{home_name} vs {away_name} (no match for {unresolved!r})")
            continue
