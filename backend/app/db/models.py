...
class AdminPick(Base):
    ...
    note: Mapped[str | None] = mapped_column(String(280), nullable=True)
    # A booking code the admin generated by hand on a real bookmaker's site --
    # unlike BookingSlip.booking_code (the removed aggregator flow), this
    # platform never generates, validates, or looks one up. It is only ever
    # whatever string the admin typed after building the identical slip
    # themselves on that bookmaker. Shown only to viewers with active
    # premium access (see routes_predictions.py's admin_picks()) -- that
    # gate is the entire commercial point of adding it.
